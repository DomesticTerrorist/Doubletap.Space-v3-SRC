...
bool C_ConfigManager::import_from_clipboard(const std::string_view file) {
...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 5; i++)
	{
		...
		setup_item(&g_cfg.legitbot.weapon[i].rcs, 0.0f, std::to_string(i) + crypt_str("Legitbot.rcs1"));
	...
	setup_item(&g_cfg.ragebot.slow_teleport, true, crypt_str("Ragebot.slow_teleport"));
	...
	setup_item(&g_cfg.ragebot.fakeduck_fix, false, crypt_str("Ragebot.fakeduck_fix"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].double_tap_shift_value, 13, std::to_string(i) + crypt_str("Ragebot.double_tap_shift_value"));
	...
	setup_item(&g_cfg.antiaim.freestanding_key, key_bind(TOGGLE), crypt_str("Antiaim.freestanding_key"));
	...
	setup_item(&g_cfg.player.visuals_others_dlight, false, crypt_str("Player.dlights"));
	...
	setup_item(&g_cfg.esp.keybinds_pos_x, 50, crypt_str("Esp.keybinds_pos_x"));
	setup_item(&g_cfg.esp.keybinds_pos_y, 50, crypt_str("Esp.keybinds_pos_y"));
	...
	setup_item(&g_cfg.misc.draw_keys, false, crypt_str("Draw.keys"));
	setup_item(&g_cfg.misc.auto_accept_matchmaking, false, crypt_str("Misc.accept"));
	...
	setup_item(&g_cfg.misc.no_pure, false, crypt_str("Misc.no_pure"));
	setup_item(&g_cfg.misc.ingame_grenade, false, crypt_str("Misc.grenade_helper"));
	setup_item(&g_cfg.misc.killvoice, false, crypt_str("Misc.kill_voice"));
	setup_item(&g_cfg.misc.spectatorlist_pos_x, 50, crypt_str("Misc.spectatorlist_pos_x"));
	setup_item(&g_cfg.misc.spectatorlist_pos_x, 50, crypt_str("Misc.spectatorlist_pos_y"));
	setup_item(&g_cfg.misc.left_knife, false, crypt_str("Misc.left_knife"));
	setup_item(&g_cfg.misc.prime, false, crypt_str("Misc.prime"));
	setup_item(&g_cfg.misc.lightingonshot, false, crypt_str("Misc.killeffectonshot"));
	...
	setup_item(&g_cfg.esp.Menucolor, Color(111, 0, 255, 255), ("Esp.Menucolor"));
	...
	setup_item(&g_cfg.misc.forceangle, false, crypt_str("Misc.forceangle"));
	setup_item(&g_cfg.misc.forceanglevalue, 0, crypt_str("Misc.forceangleamount"));
	setup_item(&g_cfg.misc.misc_sniper_crosshair, false, crypt_str("Misc.snipercrosshair"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\DoubleTap.Space\\") + config;
		}
...
void C_ConfigManager::load(std::string config, bool load_script_items)
{
	...
	zfolder = std::string(path) + ("\\DoubleTap.Space\\");
	zfolder += ("\\Inventory\\");
