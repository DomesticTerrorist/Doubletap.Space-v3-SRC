...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	//static auto roccospenis = m_cvar()->FindVar(crypt_str("Rocco_has_gay_sex_yaw"));

	Vector2D screen_origin;

	float inferno_lgbt = inferno_pizdec_lgbt->GetFloat();
	auto spawn_time = TICKS_TO_TIME(inferno->get_expiry_time());
...
{
	//auto grenade_origin = g_ctx.local()->GetAbsOrigin();
