...
void playeresp::dlight() {
	...
	for (auto i = 1; i < m_globals()->m_maxclients; i++) //-V807
	{
		...
		if (g_cfg.player.type[type].dLight)
		{
			...
			if (!e->IsDormant()) {
				...
				elight->color = g_cfg.player.type[type].dLight_color;
				...
				dlight->color = g_cfg.player.type[type].dLight_color;
				dlight->direction = getorig;
				dlight->origin = getorig;
				dlight->radius = g_cfg.player.type[type].dLightR;		//100.0f
				dlight->die_time = m_globals()->m_curtime + 0.1f;
				dlight->decay = g_cfg.player.type[type].dLightDecay;		//5.f
				dlight->key = e->EntIndex();
				dlight->style = g_cfg.player.type[type].dLightStyle;
				dlight->min_light = g_cfg.player.type[type].dLightExpoent;
			}
		}
	}
}
