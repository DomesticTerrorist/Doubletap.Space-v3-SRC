...
#if RELEASE
#if BETA
        m_cvar()->ConsoleColorPrintf(Color::Pink, crypt_str("[ DoubleTap.Space ] ")); //-V807
#else
        m_cvar()->ConsoleColorPrintf(Color::Pink, crypt_str("[ DoubleTap.Space ] "));
#endif
#else
        m_cvar()->ConsoleColorPrintf(Color::Pink, crypt_str("[ DoubleTap.Space ] ")); //-V807
#endif
...
#if RELEASE
#if BETA
        auto log = crypt_str("[ \x0CA DoubleTap.Space \x01] ") + text;
        chat->chat_print(log.c_str());
#else
        auto log = crypt_str("[ \x0C DoubleTap.Space \x01] ") + text;
        chat->chat_print(log.c_str());
#endif
#else
        auto log = crypt_str("[ \x0C DoubleTap.Space \x01] ") + text;
...
void eventlogs::add(std::string text, bool full_display)
{
    ...
    if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CONSOLE])
    {
        ...
        m_cvar()->ConsoleColorPrintf(Color(89,119,239,255), crypt_str("[DoubleTap.Space] "));
    ...
    if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
    {
        ...
        auto log = crypt_str("[ \x0CDoubleTap.Space \x01] ") + text;
        chat->chat_print(log.c_str());
    }
}*/