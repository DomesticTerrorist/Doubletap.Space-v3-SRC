...
void autowall::scale_damage(player_t* e, CGameTrace& enterTrace, weapon_info_t* weaponData, float& currentDamage)
{
	...
	if (a1 > 0 && ((i == HITGROUP_HEAD && e->m_bHasHelmet()) || (i >= HITGROUP_GENERIC && i <= HITGROUP_RIGHTARM)))
	{
		...
		if (v4)
			flNewDamage *= 0.85f;
		if (((currentDamage - currentDamage * arm_rat) * (mod * bon_mod)) > a1)
			flNewDamage = currentDamage - a1 / bon_mod;
