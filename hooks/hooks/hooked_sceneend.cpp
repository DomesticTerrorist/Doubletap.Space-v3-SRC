...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("Plagueware.xyz_chams"), nullptr), //-V807
			m_materialsystem()->FindMaterial(crypt_str("Plagueware.xyz_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Plagueware.xyz_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("Plagueware.xyz_glow"), nullptr),
			//m_materialsystem()->FindMaterial(crypt_str("wireframe"), nullptr)
		};
