...
#include "..\..\cheats\visuals\world_visuals.h"
...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	//m_engine()->ExecuteClientCmd("toggleconsole");
	...
	const char* ascii = R"ascii(
________              ___.   .__       ___________                   _________                          
\______ \   ____  __ _\_ |__ |  |   ___\__    ___/____  ______      /   _____/__________    ____  ____  
 |    |  \ /  _ \|  |  \ __ \|  | _/ __ \|    |  \__  \ \____ \     \_____  \\____ \__  \ _/ ___\/ __ \ 
 |    `   (  <_> )  |  / \_\ \  |_\  ___/|    |   / __ \|  |_> >    /        \  |_> > __ \\  \__\  ___/ 
/_______  /\____/|____/|___  /____/\___  >____|  (____  /   __/ /\ /_______  /   __(____  /\___  >___  >
        \/                 \/          \/             \/|__|    \/         \/|__|       \/     \/    \/ 
)ascii";
	...
	if (set_console)
	{
		...
		m_cvar()->ConsoleColorPrintf(Color::Pink, ascii);
		m_cvar()->ConsoleColorPrintf(Color::Green, "Last build: " __DATE__ " / " __TIME__);
	...
	if (g_ctx.local()->is_alive())
		weapon_debug_spread_show->SetValue(g_cfg.misc.misc_sniper_crosshair && !g_ctx.local()->m_bIsScoped() ? 3 : 0);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && g_ctx.globals.scoped && weapon->is_sniper())
			{
				...
				auto color = Color(g_cfg.esp.Menucolor.r(), g_cfg.esp.Menucolor.g(), g_cfg.esp.Menucolor.b(), 255);
				auto end_color = Color(g_cfg.esp.Menucolor.r(), g_cfg.esp.Menucolor.g(), g_cfg.esp.Menucolor.b(), 0);
			...
			misc::get().keybind_list();
			misc::get().get_keys();
			//misc::get().left_hand_knife();
