...
/*void render::Draw3DRainbowCircle1(int x, int y, int points, int radius, Color color)
{
	static auto hue_offset = 0.0f;
	static auto prevScreenPos = ZERO; //-V656
	static auto step = M_PI * 2.0f / 72.0f;
...
void render::filled_rect_world(Vector center, Vector2D size, Color color, int angle) {
	...
	if (!m_surface())
		return;
	//converted by arch , u can post this on ur beloved forum without give credits
	trace_t trace;
	auto surface = m_surface();
	...
	//render ->
	float anglez = math::dot_product(Vector(0, 0, 1), trace.plane.normal);
	float invanglez = math::dot_product(Vector(0, 0, -1), trace.plane.normal);
	float angley = math::dot_product(Vector(0, 1, 0), trace.plane.normal);
	float invangley = math::dot_product(Vector(0, -1, 0), trace.plane.normal);
	float anglex = math::dot_product(Vector(1, 0, 0), trace.plane.normal);
	float invanglex = math::dot_product(Vector(-1, 0, 0), trace.plane.normal);

	if (anglez > 0.5 || invanglez > 0.5)
		render::filled_rect_world(trace.endpos, Vector2D(5, 5), //hidden blend xd
			Color::Green, 0);

	else if (angley > 0.5 || invangley > 0.5)
		render::filled_rect_world(trace.endpos, Vector2D(5, 5), Color::Green, 1);

	else if (anglex > 0.5 || invanglex > 0.5)
		render::filled_rect_world(trace.endpos, Vector2D(5, 5), Color::Green, 2);
